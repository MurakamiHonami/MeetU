...
export default function MatchDetail() {
  ...
  async function submitReview() {
    ...
    setError("");
    try {
      await api.review({ matchId, rating, comment: comment.trim() || undefined });
      setReviewed(true);
            ...
            <>
              ...
              <div className="actions">
                <button
                  className="primary"
                  disabled={busy}
                  onClick={() => act(() => api.accept(matchId), "相手に通知しました")}
                >
                  話したい
                </button>
                <button
                  disabled={busy}
                  onClick={() => act(() => api.decline(matchId), "辞退しました")}
                >
                  今回は見送る
                </button>
              </div>
        ...
        <section className="panel">
          <div className="actions">
            <button
              className="teal"
              disabled={busy}
              onClick={() => act(() => api.complete(matchId), "取引を完了にしました")}
            >
              取引が終わった
            </button>
          </div>
