...
export function handleError(c: Context, e: unknown) {
  ...
  if (message.includes('当事者') || message.includes('参加者')) return apiError(c, 403, message, 'forbidden');
  if (message.includes('承諾') || message.includes('完了後') || message.includes('評価済み')) {
    return apiError(c, 409, message, 'conflict');
  }
  if (message.includes('通報') || message.includes('停止')) return apiError(c, 403, message, 'suspended');
  if (message.includes('入力') || message.includes('指定') || message.includes('送れ') || message.includes('5MB')) {
