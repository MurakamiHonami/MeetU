...
## ブランチ範囲外の作業は issue に残す

今のブランチ／PR の目的から外れるが、あとでやるべきことを見つけたら、**その場で実装しない**。
PR コメントやチャットにだけ書いて終わらせず、GitHub issue を立てて残す。

- 今の差分に混ぜるとレビューも bisect も壊れる（実例: Claude workflow 修正のついでに Docker を載せない）
- issue にしないと、次のエージェントも人間も追えない
- 今の作業にブロッカーなら、issue を先に書いてから今の PR を止めるか、スコープを明示して聞き直す
